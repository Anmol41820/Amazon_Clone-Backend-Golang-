...
func ChangePassword(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		var changePassword map[string]interface{}
		json.Unmarshal([]byte(asString), &changePassword)

		email, ok := changePassword["email"].(string)
		if !ok {
		...
		pass, ok := changePassword["password"].(string)
		if !ok {
			w.WriteHeader(http.StatusBadGateway)
			return
		}
		hex_pass, _ := hex.DecodeString(user.Password)
		presentPassword := helper.Decrypt(hex_pass, "Secret Key")
		
		if pass != string(presentPassword[:]){
			w.WriteHeader(http.StatusBadGateway)
			json.NewEncoder(w).Encode("Incorrect Password!!")
			return
		}

		newPass, ok := changePassword["newPassword"].(string)
		if !ok {
			w.WriteHeader(http.StatusBadGateway)
			return
		}
